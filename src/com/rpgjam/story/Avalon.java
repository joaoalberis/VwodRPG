...
public class Avalon {
    ...
    public boolean start() {
        ...
        Console.sleep(4000);
        Console.clearConsole();
        BattleResult battleResult = BattleSystem.startBattle(character, new Enemy("Bando Pirata", 30, 5, 50, 15),
                input, true);
        if (battleResult == BattleResult.VICTORY || battleResult == BattleResult.ESCAPE) {
            if (battleResult == BattleResult.ESCAPE) {
                ...
                Console.narrator("Vocês tentaram fugir, mas não estavam conseguindo, por sorte um monstro marinho apareceu e comeu a parte do barco onde a tripulação inimiga se encontrava. Vocês ficaram surpresos e jogaram " + character.getGold() + " ouro(s) no mar, lendas dizem que monstros marinhos  seguem coisas brilhantes!\n");
            ...
            Console.dialogf("[%s]: Quem vocês pensam que são para nos prender aqui? Se arrependam e nos soltem antes que seja tarde demais para vocês!\n", character.getNickname());
            ...
            Console.enemy("[Caçadores de Piratas]: Oque tanto vocês susurram ai?\n");
            ...
            Console.dialogf("[%s]: *Susurra* Não se preocupe!", character.getNickname());
            ...
            Console.sleep(8000);
            Console.clearConsole();
            battleResult = BattleSystem.startBattle(character, new Enemy("Caçadores de Piratas", 55, 8, 70, 35), input, true);
            if (battleResult == BattleResult.VICTORY || battleResult == BattleResult.ESCAPE) {
                if (battleResult == BattleResult.ESCAPE) {
                    int randomGold = (int) (Math.random() * 15) + 1;
                    double gold = character.getGold() * (randomGold / 100.0);
                ...
                Console.narrator("[Narrador]: Quando vocês estavam chegando no navio, a marinha aparece, um dos membros dos caçadores tinha saido antes e foi avisar a marinha, assim possibilitando que eles procurassem pelo navio de vocês!\n");
                Console.dialog("[Liz]: Pelo jeito não conseguiremos sair daqui sem um pouco de luta!\n");
                Console.sleep(5000);
                Console.clearConsole();
                battleResult = BattleSystem.startBattle(character, new Enemy("Marinha", 70, 12, 100, 50), input, true);
                if (battleResult == BattleResult.VICTORY || battleResult == BattleResult.ESCAPE) {
                    if (battleResult == BattleResult.ESCAPE) {
                        int randomGold = (int) (Math.random() * 15) + 1;
                        double gold = character.getGold() * (randomGold / 100.0);
                    ...
                    Console.dialog("[Oklama]: Sobre o carpiteiro, eu descobri que é possivel encontrar um na ilha de Blefuscu, obtive a informação que la é uma ilha feita por moradores especializados em carpitaria!\n");
